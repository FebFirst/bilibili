    ...
    var createControls = function (that, header) {
        ...
        $.each(that.columns, function (i, column) {
            ...
            if (!column.filterControl) {
                jsp.push('<div style="height: 34px;"></div>');
            } else {
                jsp.push('<div style="margin: 0 2px 2px 2px;" class="filterControl">');

                var nameControl = column.filterControl.toLowerCase();
                if (column.searchable && that.options.filterTemplate[nameControl]) {
                    addedFilterControl = true;
                    isVisible = 'visible';
                    jsp.push(that.options.filterTemplate[nameControl](that, column.field, isVisible));
                }
            }
            ...
            $.each(header.children().children(), function (i, tr) {
                ...
                if (tr.data('field') === column.field) {
                    tr.find('.fht-cell').append(jsp.join(''));
                    return false;
                }
            });
